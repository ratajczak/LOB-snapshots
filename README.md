...
```bash
sam-lob-snapshots-poloniex$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
sam-lob-snapshots-poloniex$ sam logs -n HelloWorldFunction --stack-name sam-lob-snapshots-poloniex --tail
```
...
```bash
sam-lob-snapshots-poloniex$ pip install pytest pytest-mock --user
sam-lob-snapshots-poloniex$ python -m pytest tests/ -v
```
